...
状态：**方案已定，待执行**。本文由实机测量得出结论，指定修复方向与验收标准，供后续执行者按任务逐项落地。
...
- CPU 数据为本机实测（2026-08-03，341 条 ESTABLISHED，负载正常），每组长跑 12–25 秒周期采样，参数与 stdin/stdout 组合做了交叉对照。
- 修复方案的数据完整性已验证：帧数、进程数、delta 语义均无损失。
- **未实测**：App 主进程侧的细分开销（T4 各条目的具体占比），该部分仍依赖 `PERFORMANCE_REVIEW.md` 的静态分析。
- 建议 T0 落地后用活动监视器复核一次 12 小时电源值，与本文第七节预期对照。
