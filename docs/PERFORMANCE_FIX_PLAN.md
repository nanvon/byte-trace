...
状态：**已执行并完成实机验收**（v0.1.7 落地全部任务，v0.1.8 修复两处收尾缺陷，v0.1.9 修复归属缓存两处缺陷）。执行记录见第九节，遗留事项与验收结论以第九节为准。

实机验收结果：**两个 nettop 合计 262% CPU → 单个 1.1%**，主进程 1.7%–4.7% → 0.5%。
...
- `swift build` ✅，`swift test` 43/43 ✅（v0.1.7 与 v0.1.8）→ 46/46 ✅（v0.1.9 新增 3 个用例）。
- **实机 CPU 对照已于 2026-08-03 补测完成**，见下文。

#### 实机验收数据（2026-08-03，debug 构建，35 秒运行）

| 指标 | 修复前 | 修复后 | 达标 |
| --- | ---: | ---: | :---: |
| nettop 进程数 | 2 | **1** | ✅ |
| nettop CPU | 262%（131% × 2） | **1.1%**（累计 0.38s / 34s） | ✅ |
| 主进程 CPU | 1.7%–4.7% | **0.5%**（累计 0.18s / 35s） | ✅ |
| 退出后残留 nettop | — | **0** | ✅ |

T0 的三处关键实现均正确：`stdinPipe` 强引用持有、释放顺序排在 `terminate()` + `readGroup.wait()` **之后**、父进程从不写入。

#### T2 采样间隔的准确性代价（实测澄清）

担心 `-d` 增量模式下拉长间隔会漏掉「两次采样之间建立又关闭」的短连接，故做同时段并行对照：

| 时长 | `-s 1` | `-s 5` | 差异 |
| --- | --- | --- | ---: |
| 90 秒 | 51,990,010 字节 | 44,252,682 字节 | −14.9% |
| **185 秒** | 314,939,533 字节 / 25 进程 | 306,539,387 字节 / **25 进程** | **−2.7%** |

**结论：短连接丢失可忽略，`-s 5` 对排行无影响。**

- 两种间隔下**进程数完全一致（25 vs 25），排行顺序一致**；头部（mihomo / node / Chrome）比值均为 0.97，小流量进程多数为 1.00。
- −2.7% 几乎全部来自**一次性基线帧**：`-s 5` 丢首帧 = 丢 5 秒 / 185 秒 = 2.7%，与实测吻合。基线帧只在采集器启动/重连时丢一次，**不是持续误差**，稳态下两者无实质差异。
- 90 秒那次测得的 −14.9% 是**样本过短放大了基线帧占比**所致，不代表真实误差。后续做同类对照**至少取 3 分钟**，否则结论会被基线帧污染。
...
T4 为消除 `resolve()` 的重复磁盘 I/O 新增了 `SystemProcessIdentityResolver` 内部缓存，实机验收时复查该缓存，发现两处缺陷，已修复。

**① `ResolverCache.entries` 只增不删 —— 内存单调增长**

原实现全类仅三处引用 `entries`：定义、读取、写入，**没有任何删除路径**。`cacheTTL` 只决定「能否命中」，过期条目会永久留在字典里。每个见过的 pid 占一条，Entry 含完整 `ProcessIdentity`（最多 8 层 ancestors），短命进程（shell、xpc helper、更新器）会持续堆积，上限为 pid 空间（约 10 万条）。

这正是 `PERFORMANCE_REVIEW.md` S3 描述的问题在新缓存里重现——T3 给下游 `ProcessAttributionCache` 加了 2000 条 LRU，上游却新开了一个无上限的缓存。

修复：新增 `pruneLocked(now:)`，仅在超过 `maxCacheCount`（默认 512）时整理一次——先按 TTL 清掉过期项，仍超限则丢弃 `cachedAt` 最旧的若干条。摊销成本可忽略。

**② 缓存键只有 pid —— 绕过了 PID 复用防护**

原实现以 `pid` 单独作键。`ProcessAttributionCache` 用 `(pid, processStartTime)` 作键正是为了防 pid 复用，而上游只按 pid 命中，会在 TTL 窗口内把新进程的流量记到已退出的旧进程头上；更麻烦的是返回的 identity 携带**旧的 `processStartTime`**，导致下游也用错误的键命中旧条目，**原有防护被完整绕过**。

修复：命中前调用一次 `Self.processInfo(for:)`（单次 `proc_pidinfo`）取当前启动时间，与 `entry.identity.processStartTime` 比对，不一致视为 pid 复用并重建。该 syscall 比一次完整 `buildIdentity`（最多 9 次 snapshot，含 LaunchServices 查询与读取 `Info.plist`）便宜数个数量级，缓存收益不受影响——修复后实机复测 CPU 无回退。

新增 3 个回归用例（`ProcessAttributorTests`）：缓存一致性、resolver 淘汰后仍返回相同身份、`ProcessAttributionCache` 淘汰后归属结果不变。
...
- **`ProcessAttributionCache` 的 LRU 用数组维护 `accessOrder`**，每次命中做 `firstIndex(of:)` O(n) + `remove(at:)` memmove。`CacheKey` 是 `(Int32, Date)`，比较廉价，2000 条上限下实测影响 < 0.1%，故 v0.1.9 未改动（遵循「仅最精准修复」）。若日后上限调大，应换成 O(1) 结构。
- 连接级主机名排行默认关闭，需在设置页手动开启（文档 T1 的预期取舍）。
- 保留策略默认 90 天仅对新用户生效，存量用户 `UserDefaults` 已有旧值不强制覆盖（文档 T3 注意项）。
- 基线帧间隙（每次 nettop 重启约 5 秒）流量丢失为已知取舍，非本次引入；实测占比约 2.7%（185 秒窗口），且为一次性而非持续误差。
- 活动监视器「12 小时电源」尚未复核（需应用连续运行 12 小时）。修复前为 1911.41（第二名的 6 倍），预期回落到 Little Snitch（25.30）同一量级。
