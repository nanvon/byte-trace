...
- **应用详情** —— 单个应用的总量、上下行与独立趋势图,以及该应用可观察到的主机名与识别覆盖率
- **可见主机名(实验能力)** —— 全局主机名流量排行与覆盖率指示;`nettop` 只对部分连接暴露主机名,覆盖率天然不完整;IP-only 流量统一计入「无法识别」,不展示原始 IP
- **设置** —— 分钟级数据保留策略(永不／7／30／90 天)、导出当前范围 JSON、正式统计与实验数据分别清空、显示系统进程、登录时启动
...
- 不上传、不联网回传任何统计结果
- 不解析报文内容,不解密 HTTPS,不读取请求体;主机名排行只统计 `nettop` 直接暴露的部分,属于部分可见的实验数据
